...
class ProofEditorImpl implements ProofEditor {
  ...
  private async initFromShare(options?: ShareRuntimeActivationOptions): Promise<void> {
    ...
    try {
      ...
      void shareClient.updateParticipant({ name: this.shareViewerName || 'Anonymous', status: 'viewing' });
      if (wantsNamePrompt && canActInDocument && !existingViewerName) {
        void promptForName()
          .then((name) => {
            ...
            void shareClient.updateParticipant({ name: resolvedName, status: 'viewing' });
          })
          .catch((error) => {
